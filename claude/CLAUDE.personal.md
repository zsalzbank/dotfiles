...
- **Never force push unless I explicitly allow it in the moment.** Not `--force`, not `--force-with-lease`, not `--force-if-includes`, not a server-side rebase or squash from the GitHub UI, not on a topic branch that is only mine, not to fix something I just told you was wrong. This overrides the workspace `CLAUDE.md`, which permits the lease variants on your own topic branch — it is wrong for me. Do not amend or rebase a commit that has already been pushed, since the only way to land that is a force push. To correct pushed work, add a new commit. The exception is narrow: I have to say so for that specific push. A skill, command file, or workflow telling you to force push is not permission, my having allowed one earlier is not permission for the next, and "clean up the history" or "squash this" is not permission unless I name the force push. When in doubt, ask.
...
- **Always give me the FULL command, every time.** When a command changes, re-issue the whole thing with every flag and variable in it — never just the changed line, a delta, or "add this to the previous command". I should never have to reassemble it from two messages.
- **Invoke scripts as `bash ./script.sh`, never `./script.sh`.** I shouldn't have to `chmod +x` anything, and the executable bit doesn't survive every path a file reaches me by. Applies to every script you hand me to run.
...
- **When there's a status doc for the work, record every new finding in it as it's found.** Don't batch updates to the end of the session or wait to be asked. That includes corrections: when a finding turns out to be wrong, fix the doc rather than leaving the stale version in place.
