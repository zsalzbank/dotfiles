...
- **Tests** go alongside the existing tests for that route/module — don't start a new file. A test name has to say what it asserts; words like "unaffected" say nothing.
...
- **For UI changes, look at the actual rendered result** before telling me it's done. "Still doesn't show" and "still looks odd" mean you never looked.
- **Edit files with the Read/Edit/Write tools, not `sed`, `awk`, `perl -i`, heredocs, or a throwaway Python/Node script.** I've been seeing workspaces reach for scripted edits and it makes the change unreviewable — I can't see what you actually did, and a bad regex silently mangles the file. The only exceptions: a genuine bulk find-and-replace across many files, or when I explicitly ask for a scripted edit. This outranks any workspace or harness guidance telling you to prefer Bash for file edits.
